...
// tensors 16384xf32; L,D 256xf32 (n*s).
...
module @fa attributes {hal.device.targets = [#dev]} {
  ...
  func.func @run_fwd(%Q: tensor<16384xf32>, %K: tensor<16384xf32>, %V: tensor<16384xf32>) -> (tensor<16384xf32>, tensor<256xf32>) {
    ...
    %O, %L = flow.dispatch @fwd::@flash_attention_fwd[%w](%n, %s, %d, %Q, %K, %V)
        : (i32, i32, i32, tensor<16384xf32>, tensor<16384xf32>, tensor<16384xf32>) -> (tensor<16384xf32>, tensor<256xf32>)
    return %O, %L : tensor<16384xf32>, tensor<256xf32>
  }
  func.func @run_dq(%Q: tensor<16384xf32>, %K: tensor<16384xf32>, %V: tensor<16384xf32>,
                %dO: tensor<16384xf32>, %L: tensor<256xf32>, %D: tensor<256xf32>) -> tensor<16384xf32> {
    %w = arith.constant 256 : index
    %n = arith.constant 2 : i32
    %s = arith.constant 128 : i32
    %d = arith.constant 64 : i32
    %dQ = flow.dispatch @dq::@flash_attention_bwd_dq[%w](%n, %s, %d, %Q, %K, %V, %dO, %L, %D)
        : (i32, i32, i32, tensor<16384xf32>, tensor<16384xf32>, tensor<16384xf32>, tensor<16384xf32>, tensor<256xf32>, tensor<256xf32>) -> tensor<16384xf32>
    return %dQ : tensor<16384xf32>
  }
  func.func @run_dkdv(%Q: tensor<16384xf32>, %K: tensor<16384xf32>, %V: tensor<16384xf32>,
                  %dO: tensor<16384xf32>, %L: tensor<256xf32>, %D: tensor<256xf32>) -> (tensor<16384xf32>, tensor<16384xf32>) {
    %w = arith.constant 256 : index
    %n = arith.constant 2 : i32
    %s = arith.constant 128 : i32
    %d = arith.constant 64 : i32
    %dK, %dV = flow.dispatch @dkdv::@flash_attention_bwd_dkdv[%w](%n, %s, %d, %Q, %K, %V, %dO, %L, %D)
        : (i32, i32, i32, tensor<16384xf32>, tensor<16384xf32>, tensor<16384xf32>, tensor<16384xf32>, tensor<256xf32>, tensor<256xf32>) -> (tensor<16384xf32>, tensor<16384xf32>)
    return %dK, %dV : tensor<16384xf32>, tensor<16384xf32>
  }
}
